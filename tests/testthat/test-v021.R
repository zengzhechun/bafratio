...
test_that("baf_estimate robust method matches eAppendix 13 Type I error at K=12", {
...
test_that("baf_estimate returns both p-values and defaults to plugin", {
  ...
  est_r <- baf_estimate(sim_est$logRr, sim_est$seLogRr,
...
test_that("baf_classify and ber_classify share one kernel (cross-scale equivalence)", {
  # 点估计模式：BAF 与 BER 在各自分区内部（非边界）严格等价。
  # 边界值（baf=1/3、0.5）由单独的单测覆盖（两者均判 mixed）。
  for (baf in c(0.1, 0.2, 0.4, 0.55, 0.6, 0.8, 0.91, 0.99)) {
    expect_equal(baf_classify(baf), ber_classify(baf / (1 - baf)),
                 info = sprintf("baf=%.3f point mode", baf))
  }
  # 自定义阈值的点估计等价：baf_classify(b, t) == ber_classify(b/(1-b), t/(1-t))
  for (t in c(0.4, 0.6, 0.7)) {
    b <- 0.55
    expect_equal(baf_classify(b, biasThreshold = t),
  ...
  ch <- 0.85
  expect_equal(
    baf_classify(b, ciLo = cl, ciHi = ch),
    ber_classify(b / (1 - b), ciLo = cl / (1 - cl), ciHi = ch / (1 - ch))
  )
  # 边界：BAF = 1 映射到 BER = Inf，仍判偏倚主导
  expect_equal(baf_classify(1), "bias-dominated")
  expect_equal(baf_classify(1), ber_classify(Inf))
})
...
test_that("fieller convenience wraps a fitted object", {
  ...
  manual <- baf_fieller(est$mu_bias, stats::var(boot$mu_draws, na.rm = TRUE),
                       est$log_rr_uncal, est$se_log_rr)
  from_ber <- baf_fieller(est, boot)
  from_full <- baf_fieller(fit, boot)
...
test_that("baf_bootstrap reuses the point estimate without re-fitting (P1-1)", {
  est <- baf_estimate(sim_est$logRr, sim_est$seLogRr,
                      sim_nc$logRr, sim_nc$seLogRr)
