test_that("baf_screen runs on raw inputs and returns a baf_screen object", {
  sc <- baf_screen(sim_est$logRr, sim_est$seLogRr,
                   sim_nc$logRr, sim_nc$seLogRr, nBoot = 300, seed = 42)
  expect_s3_class(sc, "baf_screen")
...
test_that("Layer 1 fails when calibrated p is at/above the threshold", {
  ...
  sc <- baf_screen(-0.05, 0.5, ncLog, ncSe, nBoot = 200, seed = 7)
...
test_that("effect-dominated scenario yields effect-evidence", {
  ...
  ncSe <- rep(0.05, 5)
  sc <- baf_screen(-0.60, 0.02, ncLog, ncSe, nBoot = 200, seed = 5)
  expect_true(sc$layer1$passed)
  expect_equal(sc$verdict$code, "effect-evidence")
})

test_that("bf_screen is a deprecated alias of baf_screen", {
  sc1 <- baf_screen(sim_est$logRr, sim_est$seLogRr,
...
test_that("baf_screen accepts a bafratio object without recomputing", {
...
test_that("summary.baf_screen returns a one-row data frame", {
  sc <- baf_screen(sim_est$logRr, sim_est$seLogRr,
                   sim_nc$logRr, sim_nc$seLogRr, nBoot = 200, seed = 42)
  sm <- summary(sc)
...
test_that("CI truncation at 0/1 is flagged as clipped", {
  # A BAF very near 1 (calibrated effect ~ 0) forces baf_ci_hi to 1 -> clipped.
  ncLog <- c(-0.30, -0.28, -0.31, -0.29, -0.27)
  ncSe <- c(0.02, 0.02, 0.02, 0.02, 0.02)
  sc <- baf_screen(-0.001, 0.01, ncLog, ncSe, nBoot = 200, seed = 11)
