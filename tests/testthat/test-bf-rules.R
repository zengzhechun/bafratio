test_that("bf_rules returns all seven rules in a fixed order", {
  r <- bf_rules(0.45, ciLo = 0.30, ciHi = 0.48, cal_p = 0.01,
...
test_that("bf_rules reproduces the manuscript rule definitions", {
  r <- bf_rules(0.45, ciLo = 0.30, ciHi = 0.48, cal_p = 0.01,
                p_bias_dom = 0.12, uncal_p = 0.001)
...
test_that("bf_rules keeps the strict/non-strict asymmetry between R2 and R3", {
  # A point estimate exactly at the threshold fails R2 (strict <) ...
  p <- stats::setNames(
    bf_rules(0.50, ciLo = 0.40, ciHi = 0.50, cal_p = 0.01)$pass,
...
test_that("bf_rules returns NA rather than FALSE for missing inputs", {
  r <- bf_rules(0.45)
  p <- stats::setNames(r$pass, r$rule)
...
test_that("C1 and C2 ignore the calibrated p-value (they are stand-alone)", {
  pass <- bf_rules(0.20, ciLo = 0.10, ciHi = 0.30, cal_p = 0.90)$pass
...
test_that("bf_rules and bf_classify answer different questions", {
  ...
  expect_identical(bf_classify(0.45, ciLo = 0.30, ciHi = 0.48), "mixed")
  p <- stats::setNames(
    bf_rules(0.45, ciLo = 0.30, ciHi = 0.48, cal_p = 0.01)$pass,
    c("R0", "R1", "R2", "R3", "R4", "C1", "C2")
  )
  expect_true(p[["R3"]])
})

test_that("bf_rules thresholds are configurable", {
  p <- stats::setNames(
    bf_rules(0.40, ciLo = 0.30, ciHi = 0.45, cal_p = 0.02,
