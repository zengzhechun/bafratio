...
test_that("qq data are sorted sample quantiles", {
  d <- baf_diagnostics(sim_nc$logRr, sim_nc$seLogRr)
  expect_equal(nrow(d$qq), 12)
  expect_true(all(diff(d$qq$sample) >= 0))
  expect_equal(d$qq$sample, sort(d$std_residuals))
})

test_that("baf_diagnostics validates inputs", {
  expect_error(baf_diagnostics(c(0.1, 0.2), c(0.1, 0.1)), ">= 3")
  expect_error(baf_diagnostics(c(0.1, NA, 0.3), c(0.1, 0.1, 0.1)), "finite")
})
