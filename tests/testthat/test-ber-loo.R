...
test_that("ber_loo bootstrap CIs are ordered and reproducible", {
  loo1 <- ber_loo(sim_est$logRr, sim_est$seLogRr,
                  sim_nc$logRr, sim_nc$seLogRr, nBoot = 100, seed = 7)
  loo2 <- ber_loo(sim_est$logRr, sim_est$seLogRr,
                  sim_nc$logRr, sim_nc$seLogRr, nBoot = 100, seed = 7)
  expect_identical(loo1$ci_lo, loo2$ci_lo)
  expect_true(all(loo1$ci_lo < loo1$ci_hi))
})

test_that("ber_loo requires at least 4 negative controls", {
  expect_error(
    ber_loo(0.1, 0.1, c(0.1, 0.2, 0.3), c(0.1, 0.1, 0.1)),
    "at least 4"
  )
})
