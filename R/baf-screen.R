...
# provenance is the v38 simulation study, see `baf_reliability`.
...
#     reference simulation calibration `baf_reliability`, both the narrow-CI and
...
#'    [baf_estimate()] is compared with `cal_p_threshold` (default 0.05). A
...
#'    calibration [baf_reliability], the probability that the *true* regime is
...
#'
#' The function returns a `baf_screen` object whose `print`/`summary` show the
...
#' [baf_reliability] reference table this screening reads.
#'
...
#' `R3`/`C2`, are available from [baf_rules()].
#'
#' @param logRr Either a `bafratio` object (from [baf_analyze()], which
...
#'   exposure-outcome pair). When a `bafratio` object is supplied, the
...
#'   ignored when a `bafratio` object is supplied, since it already has the
#'   bootstrap).
...
#'   (passed to [baf_estimate()] in the default method).
#' @param reliability Reference calibration table (default [baf_reliability]).
#'   A data frame with columns `baf_center`, `p_bias_dom_narrow`,
...
#' @param ... Ignored.
#'
#' @return An object of class `baf_screen`: a list with
#'   \describe{
#'     \item{layer1}{list with `cal_p`, `threshold`, `passed`.}
#'     \item{layer2}{list with `baf`, `baf_ci_lo`, `baf_ci_hi`,
...
#'   }
#'
#' @seealso [baf_analyze()] for the full workflow, [ber_classify()] /
#'   [baf_classify()] for the simpler three-zone (BAF-only) classification,
#'   [baf_rules()] for the full manuscript rule table, and [explain()] for the
#'   plain-English interpretation.
#'
#' @export
#' @examples
#' # raw inputs
#' sc <- baf_screen(sim_est$logRr, sim_est$seLogRr,
#'                  sim_nc$logRr, sim_nc$seLogRr, nBoot = 500, seed = 42)
#' sc
#' explain(sc)
#'
#' # or screen an existing baf_analyze() fit
#' fit <- baf_analyze(sim_est$logRr, sim_est$seLogRr,
#'                    sim_nc$logRr, sim_nc$seLogRr, nBoot = 500, seed = 42)
#' baf_screen(fit)
baf_screen <- function(logRr, seLogRr, ncLogRr, ncSeLogRr, ncNames = NULL,
                       nBoot = 2000, seed = NULL, level = 0.95,
                       method = c("plugin", "robust"),
                       reliability = baf_reliability,
                       cal_p_threshold = 0.05, ...) {
  # Accept a fitted bafratio object transparently: extract its estimate and
  # bootstrap and screen those, avoiding any recomputation.
  if (inherits(logRr, "bafratio")) {
    obj <- logRr
    est <- obj$estimate
    boot <- obj$bootstrap
    if (is.null(boot)) {
      stop("`baf_screen()` on a `bafratio` object needs the bootstrap component; ",
           "re-run `baf_analyze()` with `nBoot >= 1`.", call. = FALSE)
    }
    return(.screen_from_fit(est, boot, reliability, cal_p_threshold))
  }
  method <- match.arg(method)
  est <- baf_estimate(logRr, seLogRr, ncLogRr, ncSeLogRr,
                      ncNames = ncNames, method = method)
  boot <- baf_bootstrap(logRr, seLogRr, ncLogRr, ncSeLogRr,
...
#' @rdname baf_screen
#' @export
bf_screen <- function(logRr, ...) baf_screen(logRr, ...)

# ---- internal helpers -------------------------------------------------------

# Nearest-bin lookup of P(bias-dominated) from the reference calibration.
.lookup_pbd <- function(baf, reliability) {
  centers <- reliability$baf_center
  i <- which.min(abs(centers - baf))
  list(
    bin = i,
    baf_center = centers[i],
...
.screen_from_fit <- function(est, boot, reliability, cal_p_threshold) {
  ...
  baf <- boot$baf_median
  baf_lo <- boot$baf_ci_lo
  baf_hi <- boot$baf_ci_hi
  ci_half <- (baf_hi - baf_lo) / 2
  ...
  clipped <- (baf_lo <= 0) || (baf_hi >= 1)

  lk <- .lookup_pbd(baf, reliability)
  ...
  structure(
    list(
      layer1 = list(cal_p = cal_p, threshold = cal_p_threshold,
                    passed = layer1_pass),
      layer2 = list(baf = baf, baf_ci_lo = baf_lo, baf_ci_hi = baf_hi,
                    ci_half_width = ci_half, ci_class = ci_class,
                    clipped = clipped, bin = lk$bin, baf_center = lk$baf_center,
                    p_bias_dom_narrow = lk$p_narrow,
                    p_bias_dom_wide = lk$p_wide,
                    p_bias_dom = p_bias_dom),
      verdict = verdict,
      reliability_source = if (is.null(src)) "baf_reliability (default)" else src
    ),
    class = "baf_screen"
  )
}

# Plain-English explanation string for a baf_screen object.
.screen_explanation <- function(x) {
  ...
  out <- c(out, paste0(
    ...
    fmtNum(L2$baf, 2), " with 95% CI [", fmtNum(L2$baf_ci_lo, 2), ", ",
    fmtNum(L2$baf_ci_hi, 2), "] measures the share of the calibrated signal ",
...
#' @export
print.baf_screen <- function(x, ...) {
  L1 <- x$layer1
  L2 <- x$layer2
  V <- x$verdict
  cat("bafratio two-layer screening\n")
  cat(strrep("=", 56), "\n", sep = "")
  ...
  cat(sprintf("Layer 2  BAF = %s  [%s, %s]\n",
              fmtNum(L2$baf, 2), fmtNum(L2$baf_ci_lo, 2), fmtNum(L2$baf_ci_hi, 2)))
...
#' @export
summary.baf_screen <- function(object, ...) {
  L1 <- object$layer1
  L2 <- object$layer2
  V <- object$verdict
  structure(
    list(
      ...
      baf = L2$baf,
      baf_ci_lo = L2$baf_ci_lo,
      baf_ci_hi = L2$baf_ci_hi,
...
#'
#' Returns the human-readable explanation of a [baf_screen()] result. The
#' printed `baf_screen` object also shows this text; `explain()` gives the
#' caller the string for inclusion in reports or downstream tooling.
#'
#' @param object A `baf_screen` object.
#' @param ... Ignored.
#'
...
#' @rdname explain
#' @export
explain.baf_screen <- function(object, ...) .screen_explanation(object)

# ---- visualization ---------------------------------------------------------

#' Visualize a two-layer screening result
#'
#' Draws the [baf_screen()] decision as a two-layer diagram: the top panel
...
plot.baf_screen <- function(x, ...) {
  ...
  baf_pt <- min(max(L2$baf, 0.001), 0.999)
  baf_lbl_x <- max(0.06, min(L2$baf, 0.94))
  mid1 <- (0.60 + 0.95) / 2
  mid2 <- (0.05 + 0.40) / 2
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::annotate("segment", x = 0.5, xend = 0.5, y = 0.05, yend = 0.40,
                      color = "grey45", linetype = "dashed") +
    ggplot2::annotate("segment", x = L2$baf_ci_lo, xend = L2$baf_ci_hi,
                      y = mid2, yend = mid2, color = vcol, linewidth = 2) +
    ggplot2::annotate("segment", x = L2$baf_ci_lo, xend = L2$baf_ci_lo,
                      y = mid2 - 0.03, yend = mid2 + 0.03, color = vcol, linewidth = 1) +
    ggplot2::annotate("segment", x = L2$baf_ci_hi, xend = L2$baf_ci_hi,
                      y = mid2 - 0.03, yend = mid2 + 0.03, color = vcol, linewidth = 1) +
    ggplot2::annotate("point", x = baf_pt, y = mid2, color = vcol, size = 6) +
    ggplot2::annotate("text", x = baf_lbl_x, y = 0.025,
                      label = sprintf("BAF = %s  [95%% CI %s, %s]",
                                      fmtNum(L2$baf, 2),
                                      fmtNum(L2$baf_ci_lo, 2),
                                      fmtNum(L2$baf_ci_hi, 2)),
                      size = 3.4, fontface = "bold", hjust = 0.5, vjust = 1) +
    # ---- layer labels ----
    ...
    ggplot2::labs(
      title = "bafratio two-layer screening",
