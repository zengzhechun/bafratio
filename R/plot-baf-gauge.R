...
#'
#' @param x An object of class `bafratio` (uses its bootstrap CI and median
#'   point estimate) or `baf` (point estimate only).
...
#' plot_baf_gauge(fit)
plot_baf_gauge <- function(x, digits = 2) {
  est <- asBerObject(x)
  hasCi <- inherits(x, "bafratio")
  baf <- if (hasCi) x$bootstrap$baf_median else est$baf
  bfLo <- if (hasCi) x$bootstrap$baf_ci_lo else NA_real_
  bfHi <- if (hasCi) x$bootstrap$baf_ci_hi else NA_real_
  ...
  subtitle <- if (hasCi) {
    sprintf("BAF = %s,  %d%% CI [%s, %s]  ->  %s",
            fmtNum(baf, digits), round(x$bootstrap$level * 100),
            fmtNum(bfLo, digits), fmtNum(bfHi, digits), cls)
  } else {
    sprintf("BAF = %s  ->  %s (point estimate; run baf_bootstrap for a CI)",
            fmtNum(baf, digits), cls)
  }
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::annotate("text", x = baf, y = -0.5,
                      label = paste0("BAF = ", fmtNum(baf, digits)),
                      color = clsCol, fontface = "bold", size = 4.2) +
    ggplot2::geom_point(ggplot2::aes(x = baf, y = 0),
