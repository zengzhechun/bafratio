...
#' reported as `ber`; use [baf_bootstrap()] for the bootstrap-median point
...
#'
#' @return An object of class `baf`: a list with components
#'   \describe{
...
#'     \item{baf}{The bias attribution fraction \eqn{|\mu_B| / (|\mu_B| + |\log RR_{cal}|)},
...
#'
#' @seealso [baf_bootstrap()] for confidence intervals, [ber_classify()] for
#'   the classification rule, [baf_analyze()] for the full workflow.
#'
#' @export
#' @examples
...
#' fit
baf_estimate <- function(logRr, seLogRr, ncLogRr, ncSeLogRr, ncNames = NULL,
  ...
  if (abs(logRrCal) < 1e-10) {
    ...
    baf <- if (abs(muBias) < 1e-10) NaN else 1
  } else {
