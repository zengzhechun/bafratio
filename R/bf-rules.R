...
#
# Why this file exists: `bf_classify()` answers "which of the three zones does
...
# so the rules are exposed here rather than folded into `bf_classify()`.
...
#' `cal_p` = calibrated p-value from [ber_estimate()], `P(BD)` = probability
...
#'
#' @seealso [bf_classify()] for the three-zone classification, which answers a
#'   different question; [ber_screen()] for the two-layer screen that
...
#' bf_rules(0.45, ciLo = 0.30, ciHi = 0.48, cal_p = 0.01, p_bias_dom = 0.12)
#' bf_classify(0.45, ciLo = 0.30, ciHi = 0.48)
#'
#' # A point estimate exactly at the threshold passes R3 but not R2.
#' bf_rules(0.50, ciLo = 0.40, ciHi = 0.50, cal_p = 0.01)$pass
bf_rules <- function(baf, ciLo = NA_real_, ciHi = NA_real_,
