...
#'
#' @return An object of class `baf_boot`: a list with components
#'   \describe{
#'     \item{ci_lo, ci_hi}{Confidence limits for the BER.}
#'     \item{baf_ci_lo, baf_ci_hi}{Confidence limits for the BAF, obtained as
...
#'       [baf_estimate()]).}
#'     \item{ber_median, baf_median}{Bootstrap-median point estimates
...
#'                       nBoot = 500, seed = 42)
#' boot
baf_bootstrap <- function(logRr, seLogRr, ncLogRr, ncSeLogRr,
  ...
  # 这样既不重复跑 MLE 优化器，也避免了循环内另一次 baf_estimate 失败时
  ...
  run <- function() {
    ...
    structure(
      list(
        ci_lo = unname(ci[1]),
        ci_hi = unname(ci[2]),
        baf_ci_lo = unname(bfCiLo),
        baf_ci_hi = unname(bfCiHi),
        level = level,
        method = method,
        ber = est$ber,
        ber_median = unname(berMedian),
        baf_median = unname(bfMedian),
        boot_dist = bootBer,
