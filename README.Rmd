...
`biasratio` computes BAF and BER with logit-scale (equivalently log-scale)
...
serve as usable **effect evidence**, use `ber_screen()` (or its BER-scale
...
2. **Layer 2 (bias share).** Conditional on a real signal, `ber_screen()`
   looks up, in the reference simulation calibration `bf_reliability`, the
...
`ber_analyze()`; `ber_screen()` merely adds the Layer-1/Layer-2 decision and
the reliability lookup. For a single pair you can pass a `biasratio` object
directly (`ber_screen(fit)`), skipping the recomputation. The
`bf_reliability` table (12 BAF bins, 960,000 simulated repetitions) is shipped
with the package and is the only data `ber_screen()` needs beyond the fit.
